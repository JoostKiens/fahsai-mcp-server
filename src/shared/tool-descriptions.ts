...
export const CALL_GET_LATEST_DATE_FIRST_NOTE = `If the caller means "today"/"latest" without a specific date, call \`get_latest_date\` first — a 404 here often means the date hasn't finished ingesting yet, not that no data exists.`;
