...
export function registerGetLatestDate(server: McpServer, deps: LatestDateToolDeps): void {
  server.registerTool(
    'get_latest_date',
    {
      ...
      description: `The most recent date with complete data across fires, CAMS, and ground station readings. Call this first when a caller means "today"/"latest" without a specific date — a 404 from \`get_fires\`, \`get_weather\`, or \`get_cams\` on a bare "today" often just means that date hasn't finished ingesting yet, not that no data exists.`,
