...
export function registerGetStationReadingsHistory(
  ...
  server.registerTool(
    'get_station_readings_history',
    {
      ...
      description: `Raw PM2.5 reading time series for a single station, each point with an EPA AQI category — use this for "when exactly did it spike" questions that a daily rollup can't answer. Requires a \`station_id\` from \`get_stations\` or \`get_station_readings\` — a place name is not valid input here. \`hours\` (default 24, max ${STATION_READINGS_HISTORY_MAX_HOURS}) sets how far back to look. Note: intraday granularity depends on the station's data provider and isn't guaranteed — some stations report at most once per day.`,
