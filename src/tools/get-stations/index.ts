  ...
  server.registerTool(
    'get_stations',
    {
      ...
      description:
        ...
        'find a station near a location, then pass its `id` as `station_id` to those tools. `name` does a ' +
        'case-insensitive substring match on station name; combined with `place`/`bbox` it narrows within that ' +
        "area, used alone it searches Fahsai's full coverage area.",
