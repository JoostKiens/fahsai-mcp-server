...
- **Every tool response that includes wind data goes through the shared `wind.ts` formatter** (`fromLabel`/`toLabel`). Never format `directionDeg` inline, and never apply `+ 180` to it — that exact bug shipped twice on the Fahsai frontend (see `fahsai`'s `docs/claude/conventions.md`). `directionDeg` is always the direction wind is coming **FROM**.
- **`POST /api/explain`, `GET /health`, and urban/industrial center data are explicit non-goals**, not omissions. `/api/explain` shares Fahsai's public 500-request/day Gemini quota with real map users, is SSE-streamed, and re-wraps one LLM's synthesis inside another LLM's tool call — a bad shape for this project. This exclusion is specifically about the SSE-streamed `POST /api/explain` synthesis endpoint — the sibling `GET /api/explain/context` route (plain JSON, no Gemini call, no SSE, no shared quota) is in scope and is what `get_reading_explanation` wraps (JOO-47). `/health` has no LLM-facing value. Urban/industrial centers (`urbanSources.ts`) are static data used only inside `/api/explain`'s prompt context — there's no public route to wrap, and it's hand-maintained data with no staleness signal, so it's not a good bundling candidate either. Don't add tools for any of these without revisiting the decision explicitly.
...
- **Date-range caps are enforced client-side, matching the underlying API's own limits** (`get_fires_range`: 10 days, `get_cams_summary`: 139 days — verified live 2026-07-27, JOO-35; re-verify before changing, this cap has moved before) — reject before calling, never let the API's raw error leak through.
