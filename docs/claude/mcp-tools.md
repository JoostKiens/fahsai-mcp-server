...
- [ ] **Input schema composes the shared `locationInput` fragment** (`schemas/location.ts`) if the tool takes a location at all — never hand-roll `place`/`bbox`/`radius_km` fields per-tool.
