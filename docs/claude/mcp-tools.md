...
- [ ] **Output fields that are conditionally null/absent are documented inline** with a comment explaining the condition — e.g. `get_reading_explanation`'s `transport: null` for `OUTLIER_HIGH`/`OUTLIER_LOW` `explainCase`s is called out right above the `ScientificContextTransport` interface (`src/tools/get-reading-explanation/schema.ts`), and the full set of conditionally-null fields (including `stationBaseline: null` when the reading is unremarkable for the season) is spelled out in the tool's registered description (`src/tools/get-reading-explanation/index.ts`). A null here is meaningful signal, not missing data — don't leave the LLM to guess why a field is absent.
